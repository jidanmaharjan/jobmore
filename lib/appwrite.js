...
export const uploadFile = async (file) => {
  ...
  try {
    ...
    const fileUrl = await storage.getFileView(config.storageId, newFile.$id);
    return fileUrl;
  } catch (error) {
    throw new Error(error);
  }
...
export const savePicture = async (calorieData, imageUri) => {
  try {
    ...
    const newFile = await uploadFile(imageUri);

    const fileUrl = storage.getFileDownload(config.storageId, newFile?.$id);
